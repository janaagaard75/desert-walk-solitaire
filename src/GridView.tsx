  ...
  public render() {
    ...
    return (
      <View
        style={gridViewStyle}
      >
        {this.props.gridState.positionedCards.map(positionedCard =>
          <DraggableCardView
            draggable={this.props.gridState.draggableCards.some(card => card === positionedCard.card)}
  ...
  private handleCardDragged(card: Card, cardBoundary: Rectangle) {
    ...
    const overlappedEmptyCells = this.props.gridState.emptyCells
      .map(emptyCell => {
        return {
          cell: emptyCell,
          overlappingPixels: emptyCell.boundary.overlappingPixels(cardBoundary)
        }
      })
